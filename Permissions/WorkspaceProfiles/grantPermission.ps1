#################################################
# HelloID-Conn-Prov-Target-KPN-Lisa-Permissions-WorkspaceProfiles-Grant
# Grant workspace profile to account
# PowerShell V2
#################################################
...
try {
    ...
    #region Add account to workspace profile
    # API docs: https://mwpapi.kpnwerkplek.com/index.html, specific API call: POST /api/users/{identifier}/workspaceprofiles
    $actionMessage = "granting workspace profile [$($actionContext.References.Permission.Name)] with id [$($actionContext.References.Permission.id)] to account with AccountReference: $($actionContext.References.Account | ConvertTo-Json)"
    ...
    if (-Not($actionContext.DryRun -eq $true)) {
        ...
        $outputContext.AuditLogs.Add([PSCustomObject]@{
                # Action  = "" # Optional
                Message = "Granted workspace profile [$($actionContext.References.Permission.Name)] with id [$($actionContext.References.Permission.id)] to account with AccountReference: $($actionContext.References.Account | ConvertTo-Json)."
                IsError = $false
            })
    }
    else {
        Write-Warning "DryRun: Would grant workspace profile [$($actionContext.References.Permission.Name)] with id [$($actionContext.References.Permission.id)] to account with AccountReference: $($actionContext.References.Account | ConvertTo-Json)."
    }
    #endregion Add account to workspace profile
}
catch {
    $ex = $PSItem
    ...
    if ($auditMessage -like "*One or more added object references already exist for the following modified properties: 'members'*") {
        $outputContext.AuditLogs.Add([PSCustomObject]@{
                # Action  = "" # Optional
                Message = "Skipped $($actionMessage). Reason: User is already member of this workspace profile."
                IsError = $false
            })
    }
    else {
