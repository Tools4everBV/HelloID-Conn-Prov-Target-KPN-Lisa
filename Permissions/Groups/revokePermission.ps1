#################################################
# HelloID-Conn-Prov-Target-KPN-Lisa-Permissions-Groups-Revoke
...
try {
    ...
    if ($auditMessage -like "*ResourceNotFound*" -and $auditMessage -like "*$($actionContext.References.Permission.id)*") {
        $outputContext.AuditLogs.Add([PSCustomObject]@{
                # Action  = "" # Optional
                Message = "Skipped $($actionMessage). Reason: User is already no longer member or this group no longer exists."
                IsError = $false
            })
    }
    else {
