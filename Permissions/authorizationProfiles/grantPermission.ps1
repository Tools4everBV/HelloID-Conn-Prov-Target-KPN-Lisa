#################################################
# HelloID-Conn-Prov-Target-KPN-Lisa-Permissions-AuthorizationProfiles-Grant
# Grant authorization profile to account
# PowerShell V2
#################################################
...
try {
    ...
    #region Add account to authorization profile
    # API docs: https://mwpapi.kpnwerkplek.com/index.html, specific API call: POST /api/users/{identifier}/authorizationprofiles
    $actionMessage = "granting authorization profile [$($actionContext.References.Permission.Name)] with id [$($actionContext.References.Permission.id)] to account with AccountReference: $($actionContext.References.Account | ConvertTo-Json)"
    ...
    $grantPermissionSplatParams = @{
        ...
        Method      = "POST"
        # Body is a single string, the id of the user
        Body        = ("$($actionContext.References.Account)" | ConvertTo-Json -Depth 10)
    ...
    if (-Not($actionContext.DryRun -eq $true)) {
        ...
        $outputContext.AuditLogs.Add([PSCustomObject]@{
                # Action  = "" # Optional
                Message = "Granted authorization profile [$($actionContext.References.Permission.Name)] with id [$($actionContext.References.Permission.id)] to account with AccountReference: $($actionContext.References.Account | ConvertTo-Json)."
                IsError = $false
            })
    }
    else {
        Write-Warning "DryRun: Would grant authorization profile [$($actionContext.References.Permission.Name)] with id [$($actionContext.References.Permission.id)] to account with AccountReference: $($actionContext.References.Account | ConvertTo-Json)."
    }
    #endregion Add account to authorization profile
}
catch {
    $ex = $PSItem
    ...
    if ($auditMessage -like "*MemberAlreadyExists*") {
        $outputContext.AuditLogs.Add([PSCustomObject]@{
                # Action  = "" # Optional
                Message = "Skipped $($actionMessage). Reason: User is already member of this authorization profile."
                IsError = $false
            })
    }
    else {
