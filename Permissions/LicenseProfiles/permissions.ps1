#################################################
# HelloID-Conn-Prov-Target-KPN-Lisa-Permissions-LicenseProfiles-List
# List license profiles as permissions
# PowerShell V2
#################################################
...
try {  
    ...
    #region Get LicenseProfiles
    # API docs: https://mwpapi.kpnwerkplek.com/index.html, specific API call: GET /api/licenseprofiles
    $actionMessage = "querying license profiles"

    $kpnLisaLicenseProfiles = [System.Collections.ArrayList]@()
    do {
        $getKPNLisaLicenseProfilesSplatParams = @{
            Uri         = "$($actionContext.Configuration.MWPApiBaseUrl)/licenseprofiles"
            Method      = "GET"
            Body        = @{
                Top       = 999
                SkipToken = $Null
        ...
        if (-not[string]::IsNullOrEmpty($getKPNLisaLicenseProfilesResponse.'nextLink')) {
        ...
        $getKPNLisaLicenseProfilesResponse = $null
        $getKPNLisaLicenseProfilesResponse = Invoke-RestMethod @getKPNLisaLicenseProfilesSplatParams

        if ($getKPNLisaLicenseProfilesResponse.Value -is [array]) {
            [void]$kpnLisaLicenseProfiles.AddRange($getKPNLisaLicenseProfilesResponse.Value)
        }
        else {
            [void]$kpnLisaLicenseProfiles.Add($getKPNLisaLicenseProfilesResponse.Value)
        }
    } while (-not[string]::IsNullOrEmpty($getKPNLisaLicenseProfilesResponse.'nextLink'))

    Write-Information "Queried license profiles. Result count: $(($kpnLisaLicenseProfiles | Measure-Object).Count)"
    #endregion Get LicenseProfiles

    #region Send results to HelloID
    $kpnLisaLicenseProfiles | ForEach-Object {
