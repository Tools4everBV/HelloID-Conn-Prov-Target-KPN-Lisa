#################################################
# HelloID-Conn-Prov-Target-KPN-Lisa-Permissions-Licenses-Revoke
...
try {
    ...
    #region Remove account from license
    # API docs: https://mwpapi.kpnwerkplek.com/index.html, specific API call: DELETE /api/users/{identifier}/licenses{skuId}
    $actionMessage = "revoking license [$($actionContext.References.Permission.Name)] with skuid [$($actionContext.References.Permission.SkuId)] from account with AccountReference: $($actionContext.References.Account | ConvertTo-Json)"

    $revokePermissionSplatParams = @{
        Uri         = "$($actionContext.Configuration.MWPApiBaseUrl)/users/$($actionContext.References.Account)/licenses/$($actionContext.References.Permission.SkuId)"
        Method      = "DELETE"
    ...
    if (-Not($actionContext.DryRun -eq $true)) {
        ...
        $outputContext.AuditLogs.Add([PSCustomObject]@{
                # Action  = "" # Optional
                Message = "Revoked license [$($actionContext.References.Permission.Name)] with skuid [$($actionContext.References.Permission.SkuId)] from account with AccountReference: $($actionContext.References.Account | ConvertTo-Json)."
                IsError = $false
            })
    }
    else {
        Write-Warning "DryRun: Would revoke license [$($actionContext.References.Permission.Name)] with skuid [$($actionContext.References.Permission.SkuId)] from account with AccountReference: $($actionContext.References.Account | ConvertTo-Json)."
    }
    #endregion Remove account from license
}
catch {
    $ex = $PSItem
    ...
    if ($auditMessage -like "*User does not have a corresponding license*") {
        $outputContext.AuditLogs.Add([PSCustomObject]@{
                # Action  = "" # Optional
                Message = "Skipped $($actionMessage). Reason: User already no longer has this license."
                IsError = $false
            })
    }
    else {
